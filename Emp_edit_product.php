...
if (isset($_GET['id'])) {
    ...
    // Redirect to the productList.php page if none is provided
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if (!$has_error)
    {
        ...
        // Edit product record
        $updateProduct = updateProduct($productID, $productname, $description, $categoryID, $stockquantity, $unitprice);

        if ($updateProduct === FALSE) {
          $notification_message = '<div class="alert-red">Error: Unable to update the Product.</div>';
          $_SESSION['notification_message'] = $notification_message;
        }
        else{
          $notification_message = '<div class="alert-green">The Product has been successfully updated.</div>';
          saveLog($_SESSION['user']['UserID'], 'Edited Information of Product ID: '. $productID);
          $_SESSION['notification_message'] = $notification_message;
        }
        header('Location: http://localhost/inventory_markscent/Emp_productList.php');
        exit;
    }
   }
?>
...
<!-- /.navbar -->
  ...
  <div class="content-wrapper">
    ...
    <div class="col-md-10">
            <div>
              <a class="btn btn-primary btn-md" href="Emp_productList.php">
