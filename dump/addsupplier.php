...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if (!$has_error)
    {
        ...
        // Add Supplier record
        $addSupplier = addSupplier($suppliername, $contactnumber, $email, $address);

        $status = '';
        if ($addSupplier === FALSE) {
            $status = 'error';
			
        } elseif ($addSupplier === 'existing'){
            $status = 'existing';
        }
        else{
          saveLog($_SESSION['user']['UserID'], 'Added ' . $suppliername . ' as Supplier.');
            $status = 'success';     
        }
        header('Location: http://localhost/inventory_markscent/addSupplier.php?addSupplier_status='.$status);
        exit();
    }
   }
?>
...
<div class="wrapper">
  ...
  <div class="content-wrapper">
    ...
    <div class="col-md-10">
            ...
            <?php
              if(isset($_GET['addSupplier_status']) && $_GET['addSupplier_status'] == 'success')
          {
            echo '<div class="alert-green" >The Supplier has been successfully added.</div>';
          }
          elseif(isset($_GET['addSupplier_status']) && $_GET['addSupplier_status'] == 'error')
        {
            echo '<div class="alert-red" >Error: Unable to add the Supplier.</div>';
          }
              elseif(isset($_GET['addSupplier_status']) && $_GET['addSupplier_status'] == 'existing')
        {
            echo '<div class="alert-red" >Error: Supplier already Exist.</div>';
          }
            ?> 
              ...
              <form method="post">
                <div class="card-body">
                  <div class="form-group">
                    ...
                    <input type="text" name="suppliername" class="form-control <?php if(isset($suppliername_error_msg)) echo 'is-invalid'; ?>" placeholder="Enter Supplier Name" value="<?php if(isset($suppliername)) echo $suppliername;?>">
